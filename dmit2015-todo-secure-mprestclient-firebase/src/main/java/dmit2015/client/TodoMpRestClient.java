...
public interface TodoMpRestClient {
    ...
    JsonObject create(Todo newTodo, @QueryParam("auth") String token);

    @GET
    @Path(DOCUMENT_URL + ".json")
    LinkedHashMap<String, Todo> findAll(@QueryParam("auth") String token);

    @GET
    @Path(DOCUMENT_URL + "/{key}.json")
    Todo findById(@PathParam("key") String id, @QueryParam("auth") String token);

    @PUT
    @Path(DOCUMENT_URL + "/{key}.json")
    Todo update(@PathParam("key") String key, Todo updatedTodo, @QueryParam("auth") String token);
